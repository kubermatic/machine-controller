...
if [[ ! -z "${NUTANIX_E2E_PROXY_HOST:-}" ]]; then
  ssh -4 -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -fNT -L ${LC_PRIV_ADDR}:1${NUTANIX_E2E_PROXY_PORT}:${NUTANIX_E2E_PROXY_HOST}:${NUTANIX_E2E_PROXY_PORT} root@${LC_ADDR}
fi
