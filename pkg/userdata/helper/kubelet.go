...
{{- if .ImageRegistry }}
--pod-infra-container-image="{{ .ImageRegistry }}/machine-controller/pause:3.1" \
{{- end }}
...
{{ kubeletFlags .KubeletVersion .CloudProvider .Hostname .ClusterDNSIPs .IsExternal .ImageRegistry | indent 2 }}
...
func KubeletSystemdUnit(kubeletVersion, cloudProvider, hostname string, dnsIPs []net.IP, external bool, imageRegistry string) (string, error) {
...
func KubeletFlags(version, cloudProvider, hostname string, dnsIPs []net.IP, external bool, imageRegistry string) (string, error) {
