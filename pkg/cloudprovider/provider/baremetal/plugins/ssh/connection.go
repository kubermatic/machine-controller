...
func validateOptions(o Opts) (Opts, error) {
	...
	if len(o.Password) == 0 && len(o.PrivateKey) == 0 && len(o.KeyFile) == 0 {
...
func NewConnection(connector *Connector, o Opts) (Connection, error) {
	...
	dialledEndpoint := net.JoinHostPort(o.Hostname, strconv.Itoa(o.Port))

	// Dial a connection to the service host, from the bastion
	conn, err := client.Dial("tcp", dialledEndpoint)
	if err != nil {
		return nil, errors.Wrapf(err, "could not establish connection to %s", dialledEndpoint)
	}

	sshConfig.User = o.Username
	ncc, chans, reqs, err := ssh.NewClientConn(conn, dialledEndpoint, sshConfig)
	if err != nil {
		return nil, errors.Wrapf(err, "could not establish connection to %s", dialledEndpoint)
	}
