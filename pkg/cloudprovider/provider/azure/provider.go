...
func (p *provider) Create(machine *clusterv1alpha1.Machine, data *cloudprovidertypes.ProviderData, userdata string) (instance.Instance, error) {
	...
	addrFamily := providerCfg.Network.GetAddressFamily()
	sku := network.PublicIPAddressSkuNameBasic
	if addrFamily == util.DualStack {
	...
	iface, err := createOrUpdateNetworkInterface(context.TODO(), ifaceName(machine), machine.UID, config, publicIP, publicIPv6, addrFamily)
	if err != nil {
...
func (p *provider) Validate(spec clusterv1alpha1.MachineSpec) error {
	...
	switch f := providerConfig.Network.GetAddressFamily(); f {
