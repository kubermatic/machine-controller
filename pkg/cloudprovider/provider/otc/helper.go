...
import (
	...
	osavailabilityzones "github.com/RainbowMango/huaweicloud-sdk-go/openstack/compute/v2/extensions/availabilityzones"
	osflavors "github.com/RainbowMango/huaweicloud-sdk-go/openstack/compute/v2/flavors"
	osimages "github.com/RainbowMango/huaweicloud-sdk-go/openstack/compute/v2/images"
	osregions "github.com/RainbowMango/huaweicloud-sdk-go/openstack/identity/v3/regions"
	osfloatingips "github.com/RainbowMango/huaweicloud-sdk-go/openstack/networking/v2/extensions/layer3/floatingips"
	ossecuritygroups "github.com/RainbowMango/huaweicloud-sdk-go/openstack/networking/v2/extensions/security/groups"
	osecruritygrouprules "github.com/RainbowMango/huaweicloud-sdk-go/openstack/networking/v2/extensions/security/rules"
	osnetworks "github.com/RainbowMango/huaweicloud-sdk-go/openstack/networking/v2/networks"
	osports "github.com/RainbowMango/huaweicloud-sdk-go/openstack/networking/v2/ports"
	ossubnets "github.com/RainbowMango/huaweicloud-sdk-go/openstack/networking/v2/subnets"
...
func getRegion(client *gophercloud.ProviderClient, name string) (*osregions.Region, error) {
	idClient, err := openstack.NewIdentityV3(client, gophercloud.EndpointOpts{})
	if err != nil {
		return nil, err
	}
...
func ensureKubernetesSecurityGroupExist(client *gophercloud.ProviderClient, region, name string) error {
	...
	netClient, err := openstack.NewNetworkV2(client, gophercloud.EndpointOpts{Region: region})
	if err != nil {
		return otcErrorToTerminalError(err, "failed to get network client")
	}
	...
	if err != nil {
		if err == errNotFound {
			...
			if err != nil {
				return otcErrorToTerminalError(err, fmt.Sprintf("failed to create security group %s", name))
			}
			...
			for _, opts := range rules {
				if _, err := osecruritygrouprules.Create(netClient, opts).Extract(); err != nil {
					return otcErrorToTerminalError(err, "failed to create security group rule")
				}
			}
		}
	}
...
func getFreeFloatingIPs(client *gophercloud.ProviderClient, region string, floatingIPPool *osnetworks.Network) ([]osfloatingips.FloatingIP, error) {
	...
	for _, f := range allFIPs {
		if f.Status == floatingIPDownStatus && f.PortID == "" {
