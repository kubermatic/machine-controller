...
import (
	...
	osservers "github.com/RainbowMango/huaweicloud-sdk-go/openstack/compute/v2/servers"
	osfloatingips "github.com/RainbowMango/huaweicloud-sdk-go/openstack/networking/v2/extensions/layer3/floatingips"
	osnetworks "github.com/RainbowMango/huaweicloud-sdk-go/openstack/networking/v2/networks"
	...
	otctypes "github.com/kubermatic/machine-controller/pkg/cloudprovider/provider/otc/types"
...
const (
	floatingIPReleaseFinalizer = "kubermatic.io/release-otc-floating-ip"
	floatingIPIDAnnotationKey  = "kubermatic.io/release-otc-floating-ip"
	cloudName                  = "otc.t-systems.com"
)
...
func (c *Config) Endpoint(kind string) string {
	return fmt.Sprintf("https://%s.%s.%s", kind, c.Region, cloudName)
}
...
func (p *provider) getConfig(s v1alpha1.ProviderSpec) (*Config, *providerconfigtypes.Config, *otctypes.RawConfig, error) {
	if s.Value == nil {
		return nil, nil, nil, errors.New("machine.spec.providerconfig.value is nil")
	}
	...
	var rawConfig otctypes.RawConfig
	...
	c := Config{}
	c.AccessKey, err = p.configVarResolver.GetConfigVarStringValueOrEnv(rawConfig.AccessKey, "OS_ACCESS_KEY")
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to get the value of \"accessKey\" field: %v", err)
	}
	c.SecretKey, err = p.configVarResolver.GetConfigVarStringValueOrEnv(rawConfig.SecretKey, "OS_SECRET_KEY")
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to get the value of \"secretKey\" field: %v", err)
	}
	// Ignore Region not found as Region might not be found and we can default it later
	c.Region, err = p.configVarResolver.GetConfigVarStringValueOrEnv(rawConfig.Region, "OS_REGION")
	if err != nil {
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to get the value of \"domainName\" field: %v", err)
	}
	c.TenantID, err = p.configVarResolver.GetConfigVarStringValueOrEnv(rawConfig.TenantID, "OS_TENANT_ID")
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to get the value of \"tenantID\" field: %v", err)
	}
	c.VPCID, err = p.configVarResolver.GetConfigVarStringValueOrEnv(rawConfig.VPCID, "OS_VPC_ID")
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to get the value of \"vpcID\" field: %v", err)
	}
	c.Image, err = p.configVarResolver.GetConfigVarStringValue(rawConfig.Image)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to get the value of \"image\" field: %v", err)
	}
	c.Flavor, err = p.configVarResolver.GetConfigVarStringValue(rawConfig.Flavor)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to get the value of \"flavor\" field: %v", err)
	}
	for _, securityGroup := range rawConfig.SecurityGroups {
		...
		if err != nil {
			return nil, nil, nil, fmt.Errorf("failed to get the value of \"securityGroup\" field: %v", err)
		}
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to get the value of \"network\" field: %v", err)
	}
	c.Subnet, err = p.configVarResolver.GetConfigVarStringValue(rawConfig.Subnet)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to get the value of \"subnet\" field: %v", err)
	}
	c.FloatingIPPool, err = p.configVarResolver.GetConfigVarStringValue(rawConfig.FloatingIPPool)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to get the value of \"floatingIPPool\" field: %v", err)
	}
	c.AvailabilityZone, err = p.configVarResolver.GetConfigVarStringValue(rawConfig.AvailabilityZone)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to get the value of \"availabilityZone\" field: %v", err)
	}
	c.TrustDevicePath, err = p.configVarResolver.GetConfigVarBoolValue(rawConfig.TrustDevicePath)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("failed to get the value of \"trustDevicePath\" field: %v", err)
	}
...
func setProviderSpec(rawConfig otctypes.RawConfig, s v1alpha1.ProviderSpec) (*runtime.RawExtension, error) {
	if s.Value == nil {
...
func getClient(c *Config) (*gophercloud.ProviderClient, error) {
	opts := aksk.AKSKOptions{
		IdentityEndpoint: c.Endpoint("iam"),
...
func (p *provider) AddDefaults(spec v1alpha1.MachineSpec) (v1alpha1.MachineSpec, error) {
	...
	if err != nil {
		return spec, otcErrorToTerminalError(err, "failed to get an OTC client")
	}
	...
	if c.Region == "" {
		...
		if err != nil {
			return spec, otcErrorToTerminalError(err, "failed to get regions")
		}
		if len(regions) == 1 {
	...
	if c.AvailabilityZone == "" {
		...
		if err != nil {
			return spec, otcErrorToTerminalError(err, "failed to get availability zones")
		}
	...
	if c.Network == "" {
		...
		if err != nil {
			return spec, otcErrorToTerminalError(err, "failed to default network")
		}
		if net != nil {
	...
	if c.Subnet == "" {
		...
		if err != nil {
			return spec, otcErrorToTerminalError(err, fmt.Sprintf("failed to get network for subnet defaulting '%s", networkID))
		}
		subnet, err := getDefaultSubnet(client, net, c.Region)
		if err != nil {
			return spec, otcErrorToTerminalError(err, "error defaulting subnet")
		}
		if subnet != nil {
	...
	if err != nil {
		return spec, otcErrorToTerminalError(err, "error marshaling providerconfig")
	}
	return spec, nil
}
...
func (p *provider) Validate(spec v1alpha1.MachineSpec) error {
	...
	if c.AccessKey == "" {
		return errors.New("accessKey must be configured")
	}

	if c.SecretKey == "" {
		return errors.New("secretKey must be configured")
	}
	...
	if c.TenantID == "" {
		return errors.New("tenantID must be configured")
	}

	if c.VPCID == "" {
		return errors.New("vpcID must be configured")
	}
	...
	client, err := p.clientGetter(c)
	if err != nil {
		return fmt.Errorf("failed to get an OTC client: %v", err)
	}
	...
	if pc.OperatingSystem == providerconfigtypes.OperatingSystemSLES {
		return fmt.Errorf("invalid/unsupported operating system %q specified: %v", pc.OperatingSystem, providerconfigtypes.ErrOSNotSupported)
	}
	// Optional fields
...
func (p *provider) Create(machine *v1alpha1.Machine, data *cloudprovidertypes.ProviderData, userdata string) (instance.Instance, error) {
	...
	client, err := p.clientGetter(c)
	if err != nil {
		return nil, otcErrorToTerminalError(err, "failed to get an OTC client")
	}

	flavor, err := getFlavor(client, c.Region, c.Flavor)
	if err != nil {
		return nil, otcErrorToTerminalError(err, fmt.Sprintf("failed to get flavor %q", c.Flavor))
	}

	image, err := getImageByName(client, c.Region, c.Image)
	if err != nil {
		return nil, otcErrorToTerminalError(err, fmt.Sprintf("failed to get image %q", c.Image))
	}

	network, err := getNetwork(client, c.Region, c.Network)
	if err != nil {
		return nil, otcErrorToTerminalError(err, fmt.Sprintf("failed to get network %q", c.Network))
	}
	...
	if len(securityGroups) == 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("error occurred while creating security group: %v", err)
		}
		securityGroups = append(securityGroups, securityGroupName)
	}

	computeClient, err := openstack.NewComputeV2(client, gophercloud.EndpointOpts{Availability: gophercloud.AvailabilityPublic, Region: c.Region})
	if err != nil {
		return nil, otcErrorToTerminalError(err, "failed to get compute client")
	}
	...
	if c.RootDiskSizeGB != nil {
		...
		if err := bootfromvolume.Create(computeClient, createOpts).ExtractInto(&server); err != nil {
			return nil, otcErrorToTerminalError(err, "failed to create server with volume")
		}
	} else {
		...
		if err := osservers.Create(computeClient, serverOpts).ExtractInto(&server); err != nil {
			return nil, otcErrorToTerminalError(err, "failed to create server")
		}
	}
	...
	if err := p.serverReadinessWaiter(computeClient, server.ID); err != nil {
		...
		return nil, fmt.Errorf("instance %s did not become active: %v", server.ID, err)
	}
...
func waitUntilInstanceIsActive(computeClient *gophercloud.ServiceClient, serverID string) error {
	...
	instanceIsReady := func() (bool, error) {
		...
		if err != nil {
			tErr := otcErrorToTerminalError(err, fmt.Sprintf("failed to get current instance %s", serverID))
	...
	if err := wait.Poll(instanceReadyCheckPeriod, instanceReadyCheckTimeout, instanceIsReady); err != nil {
		if err == wait.ErrWaitTimeout {
			...
			return fmt.Errorf("instance did not become active after %f seconds", instanceReadyCheckTimeout.Seconds())
		}
...
func (p *provider) Cleanup(machine *v1alpha1.Machine, data *cloudprovidertypes.ProviderData) (bool, error) {
	...
	if err != nil {
		return false, otcErrorToTerminalError(err, "failed to get an OTC client")
	}

	computeClient, err := openstack.NewComputeV2(client, gophercloud.EndpointOpts{Availability: gophercloud.AvailabilityPublic, Region: c.Region})
	if err != nil {
		return false, otcErrorToTerminalError(err, "failed to get compute client")
	}

	if err := osservers.Delete(computeClient, instance.ID()).ExtractErr(); err != nil && err.Error() != "Resource not found" {
		return false, otcErrorToTerminalError(err, "failed to delete instance")
	}
...
func (p *provider) Get(machine *v1alpha1.Machine, _ *cloudprovidertypes.ProviderData) (instance.Instance, error) {
	...
	var allServers []osservers.Server
	pager := osservers.List(computeClient, osservers.ListOpts{Name: machine.Spec.Name})
	err = pager.EachPage(func(page pagination.Page) (bool, error) {
		var servers []osservers.Server
		err = osservers.ExtractServersInto(page, &servers)
		if err != nil {
			return false, otcErrorToTerminalError(err, "failed to extract instance info")
		}
		allServers = append(allServers, servers...)
		return true, nil
	})
	if err != nil {
		return nil, otcErrorToTerminalError(err, "failed to list instances")
	}
...
func (p *provider) MigrateUID(machine *v1alpha1.Machine, new types.UID) error {
	...
	client, err := getClient(c)
	if err != nil {
		return otcErrorToTerminalError(err, "failed to get an OTC client")
	}

	computeClient, err := openstack.NewComputeV2(client, gophercloud.EndpointOpts{Availability: gophercloud.AvailabilityPublic, Region: c.Region})
	if err != nil {
		return otcErrorToTerminalError(err, "failed to get compute client")
	}
	...
	if err != nil {
		return otcErrorToTerminalError(err, "failed to list instances")
	}
...
func (p *provider) GetCloudConfig(spec v1alpha1.MachineSpec) (config string, name string, err error) {
	...
	cc := &otctypes.CloudConfig{
		LoadBalancer: otctypes.LoadBalancerOpts{
			Apiserver:        "",
			SecretName:       "otc-ccm-credentials",
			SignerType:       "ec2",
			...
			VPCID:            c.VPCID,
			SubnetID:         c.Subnet,
			ECSEndpoint:      c.Endpoint("ecs"),
			ELBEndpoint:      c.Endpoint("elb"),
			ALBEndpoint:      c.Endpoint("elb"),
			VPCEndpoint:      c.Endpoint("vpc"),
			NATEndpoint:      c.Endpoint("nat"),
			EnterpriseEnable: "false",
		},

		Auth: otctypes.AuthOpts{
			// putting the credentials in a Secret is not yet supported in
			// the upstream cloud provider
			AccessKey:   "",
			SecretKey:   "",
			IAMEndpoint: c.Endpoint("iam"),
			DomainID:    c.DomainName,
			ProjectID:   c.TenantID,
			Region:      c.Region,
			Cloud:       cloudName,
		},
	...
	s, err := otctypes.CloudConfigToString(cc)
	if err != nil {
...
// otcErrorToTerminalError judges if the given error
...
func otcErrorToTerminalError(err error, msg string) error {
	unifiedErr, ok := err.(*gophercloud.UnifiedError)
	if ok {
		if unifiedErr.ErrCode == "Com.401" {
			return cloudprovidererrors.TerminalError{
				Reason:  common.InvalidConfigurationMachineError,
				Message: fmt.Sprintf("%s: a request has been rejected due to invalid credentials which were taken from the MachineSpec: %v", msg, unifiedErr.ErrMessage),
			}
		}

		if strings.Contains(unifiedErr.ErrMessage, "Quota exceeded") {
			return cloudprovidererrors.TerminalError{
				Reason:  common.InsufficientResourcesMachineError,
				Message: fmt.Sprintf("%s: a request cannot be fulfilled: %v", msg, unifiedErr.ErrMessage),
			}
		}
	}
...
func (p *provider) cleanupFloatingIP(machine *v1alpha1.Machine, updater cloudprovidertypes.MachineUpdater) error {
	...
	if !exists {
		return otcErrorToTerminalError(fmt.Errorf("failed to release floating ip"),
	...
	client, err := p.clientGetter(c)
	if err != nil {
		return otcErrorToTerminalError(err, "failed to get a openstack client")
	}
	netClient, err := openstack.NewNetworkV2(client, gophercloud.EndpointOpts{Region: c.Region})
	if err != nil {
...
func assignFloatingIPToInstance(machineUpdater cloudprovidertypes.MachineUpdater, machine *v1alpha1.Machine, client *gophercloud.ProviderClient, instanceID, floatingIPPoolName, region string, network *osnetworks.Network) error {
	...
	netClient, err := openstack.NewNetworkV2(client, gophercloud.EndpointOpts{Region: region})
	if err != nil {
	...
	if err != nil {
		return otcErrorToTerminalError(err, fmt.Sprintf("failed to get floating ip pool %q", floatingIPPoolName))
	}
	...
	if err != nil {
		return otcErrorToTerminalError(err, "failed to get free floating ips")
	}
	...
	if len(freeFloatingIps) < 1 {
		if ip, err = createFloatingIP(client, region, port.ID, floatingIPPool); err != nil {
			return otcErrorToTerminalError(err, "failed to allocate a floating ip")
		}
